...
exports.updateQualityParam = async (req, res) => {
  try {
    ...
    if (req.body.calculated_quantity_quintal !== undefined) {
      const newQuantity =
        qualityParam.calculated_quantity_quintal +
        req.body.calculated_quantity_quintal;

      // Ensure the sum does not exceed counter_reading
      updatedFields.calculated_quantity_quintal = Math.min(
        newQuantity,
        req.body.counter_reading || qualityParam.counter_reading
      );
    }
...
exports.rollbackQualityParam = async (req, res) => {
  try {
    ...
    if (!lastTransaction) {
      return res
        .status(404)
        .json({ error: "No update history found for rollback" });
    }
...
exports.approveQualityParam = async (req, res) => {
  try {
    const qualityParam = await QualityParams.findByPk(req.params.id);
    if (!qualityParam) {
      return res.status(404).json({ error: "Quality parameter not found" });
    ...
    const previousData = qualityParam.toJSON();

    // Update required fields
    const updatedFields = {
      icumsa: req.body.icumsa,
      temprature: req.body.temprature,
      moisture: req.body.moisture,
      retention: req.body.retention,
      approved: req.body.approved, // Mark as approved
      updated_by: userId,
      qa_approved_quantity: req.body.qa_approved_quantity,
    };

    await qualityParam.update(updatedFields);

    const newData = qualityParam.toJSON();

    // Log changes in the transaction log
    await TransactionalLogs.create({
      action: "APPROVE",
      table_name: "quality_params",
      record_id: qualityParam.id,
    ...
    res.json({
      message: "Quality parameter approved successfully",
      qualityParam,
    });
  } catch (error) {
    console.error(error);
